...
import { deleteLead, getLeadsContent, getCustomersContent } from "./customerSlice"
...
const TopSideButtons = () => {
    ...
    return(
        <div className="inline-block float-right">
            <button className="btn px-6 btn-sm btn-outline btn-primary" onClick={() => openAddNewOrderModal()}>Add New</button>
        </div>
    )
}
...
function Customers(){
    ...
    const editCurrentOrder = async(index, id) => {
        dispatch(openModal({title : "Edit Order", bodyType : MODAL_BODY_TYPES.ORDER_ADD_EDIT, extraObject: { index, id, order: Customers[index] }}))
    }
